...
private partial def parseArgsGo (args : List String)
    (airDir outPath ns prefix_ : Option String) : Except String Args :=
  match args with
  | [] =>
    match airDir, outPath, ns with
    | some airDir, some outPath, some ns =>
      .ok { airDir := airDir, outPath := outPath, ns, prefix_ := prefix_.getD "" }
    | none, _, _ => .error s!"missing <air-dir>\n{usage}"
    | _, none, _ => .error s!"missing -o <out.lean>\n{usage}"
    | _, _, none => .error s!"missing --namespace <Ns>\n{usage}"
  | "-o" :: v :: rest => parseArgsGo rest airDir (some v) ns prefix_
  | "--namespace" :: v :: rest => parseArgsGo rest airDir outPath (some v) prefix_
  | "--prefix" :: v :: rest => parseArgsGo rest airDir outPath ns (some v)
  | ["-o"] | ["--namespace"] | ["--prefix"] => .error s!"missing value for {args.head!}\n{usage}"
  | v :: rest =>
    if airDir.isNone then parseArgsGo rest (some v) outPath ns prefix_
    else .error s!"unexpected argument: '{v}'\n{usage}"

def parseArgs (args : List String) : Except String Args := parseArgsGo args none none none none
...
def main (args : List String) : IO UInt32 := do
  ...
  | .ok a =>
    ...
    if jsonPaths.isEmpty then
      ...
      | some e => die e
      | none =>
        let src := emit funcs a.ns a.prefix_
